...
class _EmprendimientosSearchPageState extends State<EmprendimientosSearchPage>
  ...
  Future<void> _initializeRepository() async {
    try {
      ...
      // ─── FIX: mark ready only after full init ───────────────────────────────
      if (mounted) {
        setState(() {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      try {
        ...
        _filterState.parroquias = _repository.getUniqueParroquias(allData).toList();
      }
      ...
      _searchState.allEmprendimientos = allData;
      _filterState.categories = _repository.getUniqueCategories(_searchState.allEmprendimientos);

      if (_filterState.parroquias.isEmpty) {
        _filterState.parroquias = _repository.getUniqueParroquias(_searchState.allEmprendimientos).toList();
      }
      ...
      if (!_isGuestUser &&
          (e.toString().contains('No autorizado') || e.toString().contains('401'))) {
        errorMessage = 'Sesión expirada. Inicia sesión nuevamente.';
        await widget.authRepository.logout();
      }
      _showErrorSnackbar(errorMessage);
    } finally {
      if (mounted) setState(() => _searchState.isLoading = false);
    }
  }
  ...
  void _filterEmprendimientos() {
    ...
    if (_searchState.currentQuery.isNotEmpty) {
      filtered = _repository.searchEmprendimientos(filtered, _searchState.currentQuery);
    }
    if (_filterState.selectedCategory != 'Todas') {
      filtered = _repository.filterByCategory(filtered, _filterState.selectedCategory);
    }
    if (_filterState.selectedParroquias.isNotEmpty) {
      filtered = filtered.where((e) => _filterState.selectedParroquias.contains(e.parroquia)).toList();
    }
    if (_filterState.maxPrice != null) {
      filtered = filtered.where((e) => e.precioPromedio <= _filterState.maxPrice!).toList();
    }
          ...
          children: [
            ...
            const Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('EmprendeGastroLoja', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color.fromARGB(255, 195, 176, 1), letterSpacing: 0.5)),
                  Text('Descubre emprendimientos', style: TextStyle(fontSize: 12, color: Color.fromARGB(255, 88, 80, 3), fontWeight: FontWeight.w500)),
                ],
              ),
            ),
            IconButton(
            ...
            PopupMenuButton<String>(
              ...
              onSelected: (value) {
                switch (value) {
                  ...
                  case 'delete': _handleDeleteAccount(); break;
                }
              },
              itemBuilder: (context) => [
                if (!_isGuestUser)
                  const PopupMenuItem(value: 'delete', child: Row(children: [Icon(Icons.delete_outline, color: Colors.red), SizedBox(width: 12), Text('Eliminar cuenta')])),
                PopupMenuItem(
                  value: 'logout',
                  child: Row(children: [const Icon(Icons.logout), const SizedBox(width: 12), Text(_isGuestUser ? 'Salir' : 'Cerrar sesión')]),
                ),
              ],
            ),
          ],
      ...
      child: SafeArea(
        child: Padding(
          ...
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceAround,
            children: [
              _buildNavItem(icon: Icons.restaurant_menu, label: 'Todos', index: 0),
              _buildNavItem(icon: Icons.star, label: 'Premium', index: 1),
              _buildNavItem(icon: Icons.favorite, label: 'Favoritos', index: 2),
            ],
          ),
        ),
      ),
  ...
  Widget _buildNavItem({required IconData icon, required String label, required int index}) {
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 8),
        decoration: BoxDecoration(gradient: isSelected ? BrandColors.gradient : null, borderRadius: BorderRadius.circular(20)),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Icon(icon, color: isSelected ? Colors.white : Colors.grey[600], size: 26),
            const SizedBox(height: 4),
            Text(label, style: TextStyle(fontSize: 12, fontWeight: isSelected ? FontWeight.bold : FontWeight.w500, color: isSelected ? Colors.white : Colors.grey[600], letterSpacing: 0.3)),
          ],
        ),
      ),
    );
    ...
    return Column(
      ...
      children: [
        if (_filterState.hasActiveFilters)
          Padding(
            ...
            child: FloatingActionButton(
              ...
              onPressed: () {
                _searchController.clear();
                setState(() { _filterState.reset(); _filterEmprendimientos(); });
              },
              child: const Icon(Icons.filter_alt_off, size: 20),
            ),
          ),
        FloatingActionButton(
          ...
          backgroundColor: Colors.white,
          child: Container(
            decoration: const BoxDecoration(gradient: BrandColors.gradient, shape: BoxShape.circle),
            child: const Center(child: Icon(Icons.tune, color: Colors.white, size: 28)),
          ),
        ),
        if (_fabAnimationController.isCompleted)
          Padding(
            padding: const EdgeInsets.only(top: 12),
            child: ScaleTransition(
              scale: Tween<double>(begin: 0.0, end: 1.0).animate(CurvedAnimation(parent: _fabAnimationController, curve: Curves.easeOut)),
              child: FloatingActionButton(
                mini: true,
                heroTag: 'scroll_top',
                backgroundColor: Colors.white,
                onPressed: () => _scrollController.animateTo(0, duration: const Duration(milliseconds: 500), curve: Curves.easeInOut),
                child: const Icon(Icons.keyboard_arrow_up, color: BrandColors.goldenYellow),
              ),
            ),
          ),
      ],
  ...
  Widget _buildEmprendimientosList(List<Emprendimiento> emprendimientos) {
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        padding: const EdgeInsets.all(16),
        itemCount: emprendimientos.length,
        itemBuilder: (context, index) => _buildEnhancedCard(emprendimientos[index]),
      ),
    );
  }
            ...
            children: [
                    ...
                    child: AspectRatio(
                      ...
                      child: Image.network(
                        ...
                        errorBuilder: (context, error, stackTrace) => Container(
                          color: Colors.grey[200],
                          child: const Icon(Icons.restaurant, size: 48, color: BrandColors.goldenYellow),
                        ),
                      ),
                    ),
                  ),
                    ...
                    child: InkWell(
                      ...
                      child: Container(
                        ...
                        child: Icon(
                          ...
                          color: emprendimiento.isFavoritedByUser ? Colors.red : Colors.grey[600],
                          size: 20,
                        ),
                      ),
                    ),
              ...
              Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(emprendimiento.nombre, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, letterSpacing: 0.3), maxLines: 2, overflow: TextOverflow.ellipsis),
                    const SizedBox(height: 8),
                    Row(
                      children: [
                        Container(
                          ...
                          child: Text(emprendimiento.categoria, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: BrandColors.deepGold)),
                        ),
                        const SizedBox(width: 8),
                        Expanded(
                          child: Row(
                            children: [
                              ...
                              const SizedBox(width: 4),
                              Expanded(child: Text(emprendimiento.parroquia, style: TextStyle(fontSize: 12, color: Colors.grey[600]), maxLines: 1, overflow: TextOverflow.ellipsis)),
                            ],
                          ),
                        ),
                      ],
                    ...
                    Row(
                      children: [
                        const Icon(Icons.person, size: 14, color: Colors.grey),
                        const SizedBox(width: 4),
                        Expanded(child: Text(emprendimiento.propietario ?? 'No especificado', style: TextStyle(fontSize: 12, color: Colors.grey[700], fontWeight: FontWeight.w500), maxLines: 1, overflow: TextOverflow.ellipsis)),
                        const SizedBox(width: 12),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                          decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(6), border: Border.all(color: Colors.blue.shade200, width: 1)),
                          child: Text(emprendimiento.tipo ?? 'N/A', style: TextStyle(fontSize: 11, color: Colors.blue.shade700, fontWeight: FontWeight.w600)),
                        ),
                      ],
                    ),
                    if (emprendimiento.oferta?.isNotEmpty ?? false) ...[
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            const Icon(Icons.local_offer, size: 14, color: Colors.orange),
                            const SizedBox(width: 6),
                            Expanded(child: Text(emprendimiento.oferta!, style: const TextStyle(fontSize: 12, color: Colors.orange, fontWeight: FontWeight.w600), maxLines: 1, overflow: TextOverflow.ellipsis)),
                          ],
                        ),
                      ),
                    ],
                      ...
                      children: [
                        Container(
                          ...
                          child: Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              const Text('\$', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.green)),
                              const SizedBox(width: 2),
                              Text(emprendimiento.precioPromedio.toStringAsFixed(2), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.green)),
                            ],
                          ),
                        ),
                        const SizedBox(width: 12),
                        Text(_getStarsFromPriority(emprendimiento.categoryPriority), style: const TextStyle(fontSize: 14)),
                        const Spacer(),
                        Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            const Icon(Icons.favorite, size: 14, color: Colors.red),
                            const SizedBox(width: 4),
                            Text('${emprendimiento.likesCount ?? 0}', style: TextStyle(fontSize: 12, color: Colors.grey[700], fontWeight: FontWeight.w500)),
                          ],
                        ),
                        const SizedBox(width: 12),
                        Row(
                          ...
                          children: [
                            const Icon(Icons.comment, size: 14, color: Colors.blue),
                            const SizedBox(width: 4),
                            Text('${emprendimiento.commentsCount ?? 0}', style: TextStyle(fontSize: 12, color: Colors.grey[700], fontWeight: FontWeight.w500)),
                          ],
                        ),
                      ],
                    ),
  ...
  List<Emprendimiento> _getPremiumEmprendimientos() {
    return _searchState.filteredEmprendimientos.where((e) => e.categoryPriority == 3).toList();
  }
  ...
  Widget _buildFavoritesList() {
          ...
          child: Column(
            ...
            children: [
              Container(
                ...
                decoration: const BoxDecoration(gradient: BrandColors.gradient, shape: BoxShape.circle),
                child: const Icon(Icons.favorite_border, size: 60, color: Colors.white),
              ),
              const SizedBox(height: 24),
              const Text('Inicia sesión para guardar favoritos', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold), textAlign: TextAlign.center),
              const SizedBox(height: 12),
              Text('Crea una cuenta para marcar tus emprendimientos favoritos', style: TextStyle(fontSize: 14, color: Colors.grey[600]), textAlign: TextAlign.center),
              const SizedBox(height: 32),
              ElevatedButton(
                onPressed: () => Navigator.pushReplacementNamed(context, '/login'),
                style: ElevatedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 32, vertical: 16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(30)), elevation: 0),
                child: Container(
                  decoration: BoxDecoration(gradient: BrandColors.gradient, borderRadius: BorderRadius.circular(30)),
    ...
    final favorites = _searchState.filteredEmprendimientos.where((e) => e.isFavoritedByUser).toList();
      ...
      return Center(
        child: Padding(
          ...
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Container(width: 100, height: 100, decoration: BoxDecoration(color: Colors.grey[100], shape: BoxShape.circle), child: Icon(Icons.favorite_border, size: 50, color: Colors.grey[400])),
              const SizedBox(height: 24),
              const Text('No tienes favoritos aún', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 8),
              Text('Explora y marca tus emprendimientos favoritos', style: TextStyle(fontSize: 14, color: Colors.grey[600]), textAlign: TextAlign.center),
              if (_filterState.hasActiveFilters) ...[
                const SizedBox(height: 24),
                TextButton.icon(
                  onPressed: () { _searchController.clear(); setState(() { _filterState.reset(); _filterEmprendimientos(); }); },
                  icon: const Icon(Icons.filter_alt_off),
                  label: const Text('Limpiar filtros'),
                  style: TextButton.styleFrom(foregroundColor: BrandColors.goldenYellow),
                ),
              ],
            ],
          ),
  ...
  void _showFilterDialog() {
    ...
    showModalBottomSheet(
      ...
      builder: (context) => FilterBottomSheet(
        ...
        allEmprendimientos: _searchState.allEmprendimientos,
        onApply: (newState) {
          setState(() => _filterState.updateFrom(newState));

          _repository.saveFilterPreferences(
            categoria: _filterState.selectedCategory,
            parroquia: _filterState.selectedParroquias.isEmpty ? 'Todas' : _filterState.selectedParroquias.join(','),
            ordenarPor: _filterState.sortBy,
          );
  ...
  void _showLogoutDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text(isGuest ? 'Salir como invitado' : 'Cerrar sesión'),
        content: Text(isGuest ? '¿Deseas salir del modo invitado? Podrás iniciar sesión después.' : '¿Estás seguro de que deseas cerrar sesión?'),
        actions: [
            ...
            onPressed: () async {
              ...
              if (isGuest) {
                ...
                try {
                  ...
                  if (mounted) _showErrorSnackbar('Error al cerrar sesión: $e');
                }
              }
            },
      ...
      result.fold(
        ...
        (_) {
          if (mounted) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Tu cuenta ha sido eliminada.'), backgroundColor: Colors.green));
          }
        },
      );
    }
  ...
  // ─── FIX: _navigateToDetail — receives and applies ALL fields from detail ───
  Future<void> _navigateToDetail(Emprendimiento emprendimiento) async {
    final result = await Navigator.push<Map<String, dynamic>>(
      context,
      MaterialPageRoute(
    ...
    if (result != null && mounted) {
      ...
      final isFavorited = result['is_favorited'] as bool;
      final isLiked = result['is_liked'] as bool;
      // ─── FIX: use the count exactly as returned by the detail page ──────────
      final likesCount = result['likes_count'] as int;

      setState(() {
        final index = _searchState.allEmprendimientos.indexWhere((e) => e.id == emprendimientoId);
  ...
  // ─── FIX: _toggleFavorite in search page — does NOT touch likesCount ────────
  Future<void> _toggleFavorite(Emprendimiento emprendimiento) async {
    debugPrint('=== Toggle Favorite (Search Page) ===');
    ...
    if (_authToken == null || _authToken!.isEmpty) {
      ...
      if (_authToken == null || _authToken!.isEmpty) {
        // ─── FIX: do NOT auto-logout/navigate, just show the error ───────────
        _showErrorSnackbar('Necesitas iniciar sesión para marcar favoritos');
        return;
      }
    }
    ...
    final originalFavoritedState = emprendimiento.isFavoritedByUser;
    final originalLikesCount = emprendimiento.likesCount;

    try {
      // Optimistic update — ONLY favorites, NOT likesCount
      setState(() {
        final index = _searchState.allEmprendimientos.indexWhere((e) => e.id == emprendimiento.id);
        if (index != -1) {
          _searchState.allEmprendimientos[index] =
              _searchState.allEmprendimientos[index].copyWith(
            isFavoritedByUser: !_searchState.allEmprendimientos[index].isFavoritedByUser,
            likesCount: originalLikesCount + (originalFavoritedState ? -1 : 1),
          );
        }
        _filterEmprendimientos();
      });
...
class FilterState {
  ...
  bool get hasActiveFilters =>
      selectedParroquias.isNotEmpty || selectedCategory != 'Todas' || maxPrice != null;
