...
class _EmprendimientosSearchPageState extends State<EmprendimientosSearchPage>
  ...
  String? _authToken;
  double? _maxPrice; // Add max price filter
  ...
  /*Future<void> _loadData() async {
    try {
      ...
      // Pass filters to getEmprendimientos
      _allEmprendimientos = await _repository.getEmprendimientos(
        ...
        forceRefresh: true, // Force refresh when filters are applied
      );
      
      try {
        _parroquias = await _repository.getParroquias();
      } catch (e) {
        _parroquias = _repository.getUniqueParroquias(_allEmprendimientos).toList();
      }
